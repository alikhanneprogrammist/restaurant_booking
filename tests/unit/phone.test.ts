...
describe('formatPhoneDraft — живой ввод с префиксом +7', () => {
  ...
  it('backspace, задевший «+», не дорисовывает 7 и не растит «777…» (регрессия)', () => {
    // Было «+77011234567», курсор в начале, backspace удалил «+» → остаток
    // без плюса. Раньше 10 цифр считались «голой вставкой» и семёрка
    // возвращалась бесконечно; со стиранием (prev длиннее) правило выключено.
    expect(formatPhoneDraft('7011234567', '+7011234567')).toBe('+7011234567');
    expect(formatPhoneDraft('77011234567', '+77011234567')).toBe('+77011234567');
    // Вставка 10 цифр (поле было коротким «+7») — код страны по-прежнему дорисовывается.
    expect(formatPhoneDraft('7011234567', '+7')).toBe('+77011234567');
  });
